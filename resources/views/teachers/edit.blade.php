            ...
            <div class="bg-gray-200 overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                <form action="{{ route('teachers.update', $teacher->id) }}" method="post">
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" placeholder="Name" name="name" value="{{ old('name') ?? $teacher->name }}"
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" placeholder="Lastname" name="lastname" value="{{ old('lastname') ?? $teacher->lastname }}"
                            class="input input-bordered w-full" style="border-radius: 8px; border-color: #bdbdbd;" />
                    </div>
                    @error('lastname')
                        <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="form-control">
                        ...
                        <input type="number" placeholder="DNI" name="dni" value="{{ old('dni') ?? $teacher->dni }}" step="0.01"
                            class="input input-bordered w-full" style="border-radius: 8px; border-color: #bdbdbd;" />
                    </div>
                    @error('dni')
                        <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" placeholder="Email" name="email" value="{{ old('email') ?? $teacher->email }}"
                            class="input input-bordered w-full" style="border-radius: 8px; border-color: #bdbdbd;" />
                        @error('email')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="modal-action">
                        <a href="/teachers"><button class="btn btn-error">
                            Cancel
                        </button></a>
